...
const programs = [
  ...
  { icon: '⌂', title: 'Shelter & emergency relief', text: 'Rapid support and safe spaces when crisis changes everything.', color: 'bg-[#f3f7f1]' },
  { icon: '✦', title: 'Education support', text: 'Opening doors to learning, dignity, and a more hopeful future.', color: 'bg-[#f8f1e6]' },
  { icon: '≈', title: 'Water & sanitation', text: 'Clean water and healthy communities that can thrive.', color: 'bg-[#eef5f5]' },
]

const stories = [
  { image: '/images/somali-water-well.png', alt: 'Somali community building a water well', tag: 'Field update', title: 'Clean water brings health, dignity, and hope' },
  { image: '/images/somali-food-distribution.png', alt: 'Somali families receiving food support', tag: 'Community story', title: 'Sharing food, strengthening families, restoring dignity' },
  { image: '/images/somali-girls-sewing.png', alt: 'Somali girls learning sewing and tailoring together', tag: 'Skills & livelihoods', title: 'Learning skills, creating opportunity together' },
  { image: '/images/somali-mother-child.png', alt: 'Somali mother embracing her child', tag: 'Family story', title: 'Care and connection help families heal' },
  { image: '/images/somali-smiling-child.png', alt: 'Smiling Somali child', tag: 'A brighter future', title: 'Every child deserves a reason to smile' },
]
...
export default function Page() {
  ...
  const { data: impact } = useSWR('/api/impact', (url) => fetch(url).then((response) => response.json()))
  ...
  async function startDonation() {
    ...
    try {
      const response = await fetch('/api/create-checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: numericAmount }) })
